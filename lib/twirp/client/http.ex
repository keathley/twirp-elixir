defmodule Twirp.Client.HTTP do
  ...
  defstruct [service_url: nil, headers: []]

  def new(url, headers) do
    %__MODULE__{service_url: url, headers: headers}
  end
  ...
  defimpl Twirp.Client.Callable do
    ...
    def call(client, rpcdef, req, opts) do
      ...
      case Mojito.post(path, client.headers, encoded_payload, opts) do
        {:error, %{reason: %Mint.TransportError{reason: reason}}} ->
        ...
        {:ok, %{status_code: status}=env} when status != 200 ->
          {:error, build_error(env, rpcdef)}

        {:ok, %{status_code: 200}=env} ->
